...
class OWUndulatorPySRU(WofryWidget, WidgetDecorator):
    ...
    def receive_syned_data(self, data):    #     if not data is None:
    ...
    def receive_trigger_signal(self, trigger):
        if trigger and trigger.new_object == True:
            if trigger.has_additional_parameter("variable_name"):
                ...
                self.send_wofry_data()

    def send_wofry_data(self):
        ...
        if self.flag_send_wavefront_dimension == 0:
            self.Outputs.wofry_data.send(
                WofryData(wavefront=self.calculated_wavefront, beamline=beamline))
        elif self.flag_send_wavefront_dimension == 1:  # H
            self.Outputs.wofry_data.send(
                WofryData(wavefront=self.calculated_wavefront.get_Wavefront1D_from_profile(0, 0.0),
                          beamline=beamline))
        elif self.flag_send_wavefront_dimension == 2:  # V
            self.Outputs.wofry_data.send(
                WofryData(wavefront=self.calculated_wavefront.get_Wavefront1D_from_profile(1, 0.0),
                          beamline=beamline))
    ...
    def do_plot_results(self, progressBarValue):
        ...
        if isinstance(wf, GenericWavefront2D):
            ...
            self.plot_data2D(wf.get_intensity(),
                             ...
                             20, 0, 0,
                             title=self.titles[0],
                             xtitle=r"Spatial Coordinate H [$\mu$m]",
                             ytitle=r"Spatial Coordinate V [$\mu$m]")
