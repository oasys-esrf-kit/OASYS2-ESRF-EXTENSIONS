...
class OWstepper1D(WofryWidget):
    ...
    class Inputs:
        ...
        trigger    = Input("Trigger", TriggerIn, id="Trigger", auto_summary=False)

    class Outputs:
        trigger = Output("Trigger", TriggerOut, id="Trigger", default=True, auto_summary=False)
    ...
    def __init__(self):
        ...
        self.le_current_new_object.setStyleSheet("color: darkblue; font-weight: bold; background-color: rgb(243, 240, 160);")
    ...
    def do_plot_results(self, progressBarValue=50):
        if self.accumulated_data is None:
            ...
            self.plot_data1D(x=1e6*self.accumulated_data["x"],
                             ...
                             title=self.titles[0],
                             calculate_fwhm=True,
                             xtitle=r"Spatial Coordinate [$\mu$m]",
                             ytitle="Intensity")
            ...
            if nruns > 1:
                self.plot_data2D(all_scans, x, 1e6*self.accumulated_data["x"],
                                 ...
                                 xtitle="%s [%s]" % (self.variable_display_name, self.variable_um),
                                 ytitle=r"Spatial Coordinate [$\mu$m]",)
    ...
    def startLoop(self):
        ...
        if do_loop:
            ...
            self.Outputs.trigger.send(TriggerOut(new_object=True, additional_parameters={"variable_name": self.variable_name,
