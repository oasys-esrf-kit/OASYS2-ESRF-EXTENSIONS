...
class OWWOScanningScreen1D(OWWOOpticalElement1D):
    ...
    def __init__(self,is_automatic=True, show_view_options=True, show_script_tab=True):
        ...
        button.setStyleSheet("color: darkred; font-style: italic; height: 45px;")
    ...
    def propagate_wavefront(self):
        ...
        if not isinstance(wavefront, GenericWavefront1D):
            QMessageBox.critical(self, "Error",
                                 "Input wavefront must be 1D (this is a 1D widget).", QMessageBox.Ok)
            return
        ...
        if self.file_flag:
            ...
            h5w.add_dataset(qs, fwhm*1e6, dataset_name="fwhm", entry_name=None, title_x="q [m]", title_y=r"fwhm [$\mu$m]")
            h5w.add_image(intensities, image_x=qs,image_y=x*1e6,image_name="scan",entry_name=None,title_x="q [m]",
                          title_y=r"Spatial Coordinate [$\mu$m]")
